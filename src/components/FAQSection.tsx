          ...
          <Accordion type="single" collapsible className="w-full border-t border-border/70">
            {faqItems.map((item) => (
              <AccordionItem key={item.question} value={item.question} className="border-border/70">
                <AccordionTrigger className="py-5 text-left font-serif text-lg font-semibold leading-snug text-foreground hover:no-underline data-[state=open]:text-gold-dark sm:py-6 sm:text-2xl">
                  {item.question}
                </AccordionTrigger>
